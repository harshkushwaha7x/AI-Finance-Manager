...
import type { IncomeSavedView, IncomeSavedViewId } from "@/types/income";

type IncomeSavedViewsProps = {
  views: IncomeSavedView[];
  activeView: IncomeSavedViewId;
  onSelect: (viewId: IncomeSavedViewId) => void;
};

export function IncomeSavedViews({
  views,
  activeView,
  onSelect,
}: IncomeSavedViewsProps) {
  return (
    <section className="space-y-4">
      <div>
        ...
        <h3 className="mt-3 font-display text-3xl font-bold tracking-tight text-foreground">
          Jump straight to the most useful cash-in slices
        </h3>
      </div>
