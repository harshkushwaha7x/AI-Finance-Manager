...
import type { ExpenseSavedView, ExpenseSavedViewId } from "@/types/expenses";

type ExpenseSavedViewsProps = {
  views: ExpenseSavedView[];
  activeView: ExpenseSavedViewId;
  onSelect: (viewId: ExpenseSavedViewId) => void;
};

export function ExpenseSavedViews({
  views,
  activeView,
  onSelect,
}: ExpenseSavedViewsProps) {
  return (
    <section className="space-y-4">
      <div>
        ...
        <h3 className="mt-3 font-display text-3xl font-bold tracking-tight text-foreground">
          Jump between the most useful expense slices
        </h3>
      </div>
