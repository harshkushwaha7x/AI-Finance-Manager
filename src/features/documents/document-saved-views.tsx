...
import type { DocumentSavedView, DocumentSavedViewId } from "@/types/documents";

type DocumentSavedViewsProps = {
  views: DocumentSavedView[];
  activeView: DocumentSavedViewId;
  onSelect: (viewId: DocumentSavedViewId) => void;
};

export function DocumentSavedViews({
  views,
  activeView,
  onSelect,
}: DocumentSavedViewsProps) {
  return (
    <section className="space-y-4">
      <div>
        ...
        <h3 className="mt-3 font-display text-3xl font-bold tracking-tight text-foreground">
          Move between upload slices without losing workflow context
        </h3>
      </div>
